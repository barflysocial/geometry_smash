...
function renderMusic(music){
  const panel=$('musicPanel'); const staff=$('staffBox'); const play=$('playSoundBtn');
  if(!music){ panel.classList.add('hidden'); staff.innerHTML=''; return; }
  panel.classList.remove('hidden'); staff.innerHTML=renderStaffMarkup(music); play.classList.toggle('hidden', !music.audio);
}
...
async function playMusic(){
  const music=state.question?.music; if(!music?.audio) return;
  ...
  const tempo=music.audio.tempo||96; let t=ctx.currentTime+0.05;
  for(const n of music.audio.notes||[]){
